...
return {
	...
	config = function()
		...
		map("n", "<leader>fg", builtin.live_grep, { noremap = true, silent = true, desc = "File grep" })
		map("n", "<leader>fb", ":Telescope buffers<CR>", { noremap = true, silent = true, desc = "Telescope buffers" })
		map("n", "<leader>rg", builtin.registers, { noremap = true, silent = true, desc = "Telescope registers" })
		-- good practice to ues above args in {} or opts
		map(
			"n",
			"<leader>th",
			":Telescop colorscheme<CR>",
			{ noremap = true, silent = true, desc = "Telescope colorschemes" }
		)
		map(
			"n",
			"<leader>df",
